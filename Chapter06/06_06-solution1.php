...
PART3;
		...
		/*
		   - use a foreach loop to process each ID passed in the
		   '$_GET' superglobal
		   - for each one, a unique query must be crafted, as
		   shown below
		   - the '$result' need only be executed; you don't have
		   to fetch anything because the record is removed
		*/
		foreach( $todelete as $d ) {
			// each query is unique for the ID removed
			$result = $db->prepare("DELETE FROM `users` WHERE `id` = '$d'");
			$result->execute();
			// update the user
			print "<p>Removing ID$d</p>";
		}
		// optionally let them return to the main page:
		print "<p><a href=\"$thisfile\">Review user list</a></p>";
	}
?>
</body>
</html>
